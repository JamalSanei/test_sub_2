...
async function createAdminPermission(
    roles: { superAdminRole: Role; adminRole: Role; userRole: Role },
    createdRoutes: Route[],
) {
    const routeIds = createdRoutes.map((route) => ({ id: route.id }))
    const loginRoutes = filterRoutes(createdRoutes, (item: Route) =>
        item.address.includes('login'),
    )
    const orderRoute = filterRoutes(createdRoutes, (item: Route) => {
        if (
            item.address === '/api/v1/order' &&
            item.method === $Enums.RequestMethod.GET
        )
            return item
    })
